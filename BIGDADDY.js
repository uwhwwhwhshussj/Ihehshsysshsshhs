    ...
    PhistarBotInc.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            if (mek.key.remoteJid === 'status@broadcast') {
                ...
                if (now - lastStatus < 2000) { // Increased to 2 seconds to prevent rate-limiting issues
            ...
            if (['imageMessage', 'videoMessage'].includes(mtype)) {
                ...
                if (now - lastMedia < 2000) { // Increased to 2 seconds to prevent rate-limiting issues
            ...
            // Note: Check BigDaddyHandler for async operations or heavy processing that might cause delays
